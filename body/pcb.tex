...
如\hyperlink{command_mapping}{\color{blue}\uline{附录}}中所示，可以在 HELP 文档中搜索  COMMAND 找到 COMMAND MAPPING 来查找操作对应命令。如\ref{command_map}所示。
...
\begin{figure}[H]
  % Requires \usepackage{graphicx}
  \centering
  \includegraphics[width=6cm]{pick}\\
  \caption{改变器件定位点}\label{pick_component}
\end{figure}
...
\begin{enumerate}
  ...
  \item 不能在 PCB 中单独放一个焊盘，必须为这个焊盘再建一个 DRA 文件，而且每个 DRA 封装都必须有PIN1脚。
      对应每个安装孔都是有 DRA 文件的。里面可能就一个焊盘。必须为 CONNECT PIN
\end{enumerate}


\subsection{allegro 参数设置}

\subsubsection{显示，关闭所有层}

有时为了发现错误，不知道在哪一层，可以在 Display $\rightarrow$ color/visiblity $\rightarrow$ global visibility 选择 ON 和 off 来进行所有层显示的切换
...
的库路径内，否则打开转出的文件会看不到焊盘，注意保持英文和无空格路径。
             \item  必须放在英文目录下ORCAD才能导入PROTEL PCB
             \item 必须在.PCBDOC中加入封装，再转换成.PCB文件才能使用，不能直接在.PCB中加入封装
              \item export 的选项要全部勾上，特别是 No Library Dependencies，否则生成封装找不到焊盘
          \end{enumerate}
}
\color{black}
...
\subsubsection{xnet 设置}
  在 Constraint Manager 里可以设置 XNET，但是需要将对应的元器件都建立模型。在\fbox{Analyze} $\rightarrow$ \fbox{SI EMI SIM} $\rightarrow$ \fbox{Module} $\rightarrow$ 选择一个 xnet 中用到的元器件参数，输入任意值 $\rightarrow$ 然后 autosetup，确定
  ...
  \begin{enumerate}
    ...
    \includegraphics[width=4cm]{xnet_1}

    Constraint Manager 显示：

    \includegraphics[width=4cm]{xnet_2}

    SIGXPLORER

    \includegraphics[width=6cm]{xnet_3}\quad\includegraphics[width=6cm]{xnet_3_2}

    \item  建立模型：找到 Xnet 中用到的器件，按本节方法生成 Module

    \includegraphics[width=6cm]{xnet_4}

    \item  自动生成xnet:

    \includegraphics[width=4cm]{xnet_5} \quad  \includegraphics[width=4cm]{xnet_6}

    \includegraphics[width=10cm]{xnet_7}
  \end{enumerate}


\subsubsection{相对等长设置}
...
\includegraphics[width=12cm]{relative_11}

  \item 管理器中显示所有长度数值：

\includegraphics[width=6cm]{relative_1}\quad
\includegraphics[width=6cm]{relative_2}

\end{enumerate}

\paragraph{相对等长约束设置}
\begin{enumerate}
  \item 建立组：选取要等长的一组线，创建组。

\includegraphics[width=6cm]{relative_set1}
  \item 设置相对约束：也可以在 SigXplorer 里设置约束。

\includegraphics[width=6cm]{relative_set2}\quad
  \item 设置 Target 线：默认的 target 线按以下规则设定，可以在 SigXplorer 里单独对最长的线将 delta 值改为 None。
  \begin{itemize}
    \item 有且只有一对 pin-pair delta 的值设为 None（\textcolor[rgb]{1.00,0.00,0.00}{不是 0}），此线为 Target。
    \item 所有 pin-pair 都有 delta 值，则 delta 值最小的线为 Target
    \item 多个 delta 值最小，则长度最长的 pin pair 线为 Target
    \item 多个 delta 值为 None，则 长度最长的 pin pair 线为 Target
  \end{itemize}
\end{enumerate}
